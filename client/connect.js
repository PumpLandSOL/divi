...
(function () {
  ...
  async function connect(eager) {
    ...
    if (!eth) { if (!eager) alert('No EVM wallet found — install MetaMask or Rabby. The practice desk still works with a local desk id.'); return; }
    try {
      ...
      localStorage.diviConnected = '1'; setLive(acc[0]);
    } catch (e) {}
  }
  ...
  if (localStorage.diviConnected === '1') connect(true);
  // local desk id for wallet-less visitors
  window.deskId = function () {
    if (window.WALLET) return window.WALLET;
    let id = localStorage.diviDesk;
    if (!id) { const b = new Uint8Array(8); crypto.getRandomValues(b); id = 'desk_' + Array.from(b).map((x) => x.toString(16).padStart(2, '0')).join(''); localStorage.diviDesk = id; }
    return id;
  };
})();
