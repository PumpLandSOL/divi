// DIVI ($DIVI) — perps on tokenized stocks that pay real dividends. Robinhood Chain, 24/7.
// Shorts pay the dividend, longs receive it, pro rata by shares, on the ex-date. Nothing minted, nothing subsidised.
// Dependency-free Node: price+dividend engine (server/engine.js), real sources (server/sources.js), secp256k1 signing (server/evm.js), SSE board.
'use strict';
...
// ── persistence ───────────────────────────────────────────────────────────────
let DB = { signer: null, evm: null, hist: null, div: null, desk: {}, startedAt: Date.now() };
...
E.importHist(DB.hist); E.importDiv(DB.div);
let dirty = false;
function save() { try { DB.hist = E.exportHist(); DB.div = E.dividends(); fs.writeFileSync(DATA_PATH, JSON.stringify(DB)); dirty = false; } catch (e) { console.error('save failed', e.message); } }
setInterval(() => { if (dirty) save(); }, 15e3);
setInterval(save, 120e3);

// ── dividend view helpers ─────────────────────────────────────────────────────
function divView(sym, mark) {
  const d = E.dividend(sym); if (!d) return null;
  const t = Date.now();
  const yieldPct = mark ? d.annual / mark * 100 : null;
  return { annual: d.annual, perYear: d.perYear, yieldPct, next: d.next, last: d.last, history: d.history, daysToEx: d.next ? (d.next.exTs * 1000 - t) / 86400e3 : null, updated: d.updated };
}
// the record the contract accepts: only CONFIRMED (exchange-declared) dividends are signed
function signedDiv(sym) {
  const d = E.dividend(sym); if (!d || !d.next || !d.next.confirmed) return null;
  return Object.assign({ confirmed: true, ex: d.next.ex, pay: d.next.pay, amountUsd: d.next.amount }, EVM.signDividend(sym, d.next.amount, d.next.exTs));
}
...
function acct(w) {
  ...
  if (!DB.desk[w]) { DB.desk[w] = { cash: DESK_START, positions: [], closed: [], divReceived: 0, divPaid: 0, openedAt: Date.now() }; dirty = true; }
  return DB.desk[w];
}
...
function markDesk() {
  ...
  for (const w of Object.keys(DB.desk)) {
    ...
    for (const p of a.positions.slice()) {
      ...
      // dividend settlement: confirmed record, ex-date reached, position predates it, not yet settled for it
      const d = E.dividend(p.sym);
...
function leaderboard() {
  return Object.entries(DB.desk).map(([w, a]) => { const v = deskView(w); return { w, equity: v.equity, divReceived: v.divReceived, trades: a.closed.length + a.positions.length }; }).filter((x) => x.trades > 0).sort((a, b) => b.equity - a.equity).slice(0, 12);
}
...
function boardView() {
  const t = Date.now(), sess = E.session(t);
  return {
    t, session: sess, nextOpen: E.nextOpen(t),
    feeds: E.snapshot().map((m) => ({ sym: m.sym, name: m.name, mark: m.mark, conf: m.conf, confBps: m.confBps, session: m.session, official: m.official, prevClose: m.prevClose, basis: m.basis, poolLiq: m.poolLiq, poolCount: m.poolCount, seq: m.seq, ts: m.ts, dir: m.dir, nSrc: m.sources.length, nRej: m.rejected.length, div: divView(m.sym, m.mark) })),
    status: E.STATUS, signer: E.pubkeyHex(), evmSigner: EVM.address(), live: LIVE, mint: DIVI_MINT || null, perps: EVM.PERPS || null,
  };
}
let LIQ = [];
async function scanLiq() {
  if (!EVM.PERPS) return;
  try { const ps = await EVM.openPositions(); const st = await EVM.state(''); LIQ = ps.filter((p) => { const m = E.feed(p.sym); if (!m) return false; const d = (m.mark - p.entry) / p.entry * (p.isLong ? 1 : -1); return p.margin + p.size * d <= p.size * (st.maintBps || 500) / 1e4; }).map((p) => ({ id: p.id, sym: p.sym })); } catch (e) {}
}
setInterval(scanLiq, 30e3);
let lastBoard = null;
function loop() { const t = Date.now(); E.tick(t); markDesk(); lastBoard = boardView(); broadcast(lastBoard); dirty = true; }
...
function file(res, p) { fs.readFile(p, (err, buf) => { if (err) { res.writeHead(404, { 'content-type': 'text/plain' }); return res.end('not found'); } res.writeHead(200, { 'content-type': MIME[path.extname(p)] || 'application/octet-stream', 'cache-control': 'no-cache' }); res.end(buf); }); }
...
const cfgOf = (sym) => E.BOARD.find((b) => b.sym === sym);

const server = http.createServer(async (req, res) => {
  const url = new URL(req.url, 'http://x'); const u = url.pathname; let m;
  ...
  if (u === '/api/stream') { res.writeHead(200, { 'content-type': 'text/event-stream', 'cache-control': 'no-cache', connection: 'keep-alive', 'access-control-allow-origin': '*' }); res.write('data: ' + JSON.stringify(lastBoard || boardView()) + '\n\n'); clients.add(res); req.on('close', () => clients.delete(res)); return; }
  if ((m = u.match(/^\/api\/feed\/([A-Za-z]+)$/))) {
    const f = E.feed(m[1].toUpperCase()); if (!f) return json(res, 404, { error: 'unknown feed' });
    const c = cfgOf(f.sym), fut = E.FUT[c.fut];
    return json(res, 200, Object.assign({}, f, { div: divView(f.sym, f.mark), xstock: E.xsOf(f.sym), rh: c.rh, history: E.history(f.sym), daily: E.daily(f.sym), futures: fut ? { sym: c.fut, price: fut.price, ts: fut.ts } : null }));
  }
  if ((m = u.match(/^\/api\/feed\/([A-Za-z]+)\/signed$/))) { const f = E.feed(m[1].toUpperCase()); if (!f) return json(res, 404, { error: 'unknown feed' }); return json(res, 200, Object.assign({ sym: f.sym, mark: f.mark, conf: f.conf, ts: f.ts, session: f.session, seq: f.seq }, f.signed)); }
  if ((m = u.match(/^\/api\/feed\/([A-Za-z]+)\/evm$/))) { const f = E.feed(m[1].toUpperCase()); if (!f) return json(res, 404, { error: 'unknown feed' }); return json(res, 200, Object.assign({ mark: f.mark, confUsd: f.conf, fillLong: f.mark + f.conf / 2, fillShort: f.mark - f.conf / 2 }, EVM.signPrice(f))); }
  if (u === '/api/dividends') { const o = {}; for (const b of E.BOARD) { const f = E.feed(b.sym); o[b.sym] = Object.assign({ name: b.name, mark: f ? f.mark : null }, divView(b.sym, f ? f.mark : null) || {}); } return json(res, 200, { t: Date.now(), dividends: o }); }
  if ((m = u.match(/^\/api\/dividend\/([A-Za-z]+)$/))) { const sym = m[1].toUpperCase(); if (!cfgOf(sym)) return json(res, 404, { error: 'unknown' }); const f = E.feed(sym); return json(res, 200, Object.assign({ sym }, divView(sym, f ? f.mark : null) || {}, { signed: signedDiv(sym) })); }
  if ((m = u.match(/^\/api\/dividend\/([A-Za-z]+)\/evm$/))) { const sym = m[1].toUpperCase(); const s = signedDiv(sym); return s ? json(res, 200, s) : json(res, 404, { error: 'no confirmed dividend record for ' + sym + ' yet' }); }
  ...
  for (const page of ['trade', 'calendar', 'desk', 'docs']) if (u === '/' + page) return file(res, path.join(CLIENT, page + '.html'));
  if ((m = u.match(/^\/feed\/([A-Za-z]+)$/))) return file(res, path.join(CLIENT, 'feed.html'));
  if (u === '/contracts/DiviPerps.sol') return file(res, path.join(__dirname, '..', 'contracts', 'DiviPerps.sol'));
  if (u === '/keccak.js') return file(res, path.join(__dirname, '..', 'shared', 'keccak.js'));
  if (u.startsWith('/brand/')) { const p = path.join(__dirname, '..', 'brand', path.basename(u)); if (fs.existsSync(p)) return file(res, p); }
  const safe = path.normalize(u).replace(/^(\.\.[\/\\])+/, ''); const p = path.join(CLIENT, safe);
  if (p.startsWith(CLIENT) && fs.existsSync(p) && fs.statSync(p).isFile()) return file(res, p);
  res.writeHead(404, { 'content-type': 'text/plain' }); res.end('not found');
});

E.start().then(() => {
  loop(); setInterval(loop, 4000); scanLiq();
  server.listen(PORT, () => console.log('DIVI on :' + PORT + ' — ' + E.BOARD.length + ' dividend names · session ' + E.session(Date.now()) + ' · evm signer ' + EVM.address() + ' · perps ' + (EVM.PERPS || 'not set')));
});
