// DIVI — source adapters (forked from MARK). Every adapter returns plain {price, ts, ...} objects or null.
...
// ── dividends ─────────────────────────────────────────────────────────────────
// Nasdaq: confirmed upcoming ex-date, pay date, cash amount, annualized dividend (keyless, needs a browser UA)
async function nasdaqDividends(sym, etf) {
  const j = await getJSON('https://api.nasdaq.com/api/quote/' + encodeURIComponent(sym) + '/dividends?assetclass=' + (etf ? 'etf' : 'stocks'), { 'accept-language': 'en-US,en;q=0.9', origin: 'https://www.nasdaq.com', referer: 'https://www.nasdaq.com/' }, 12000);
  const d = j && j.data; if (!d) return null;
  const mdy = (s) => { const m = /^(\d\d)\/(\d\d)\/(\d{4})$/.exec(s || ''); return m ? Date.UTC(+m[3], +m[1] - 1, +m[2]) : null; };
  const rows = ((d.dividends && d.dividends.rows) || []).filter((r) => r.type === 'Cash' && /^\$/.test(r.amount || '')).map((r) => ({ ex: mdy(r.exOrEffDate), pay: mdy(r.paymentDate), amount: +r.amount.replace(/[$,]/g, '') })).filter((r) => r.ex);
  return { ex: mdy(d.exDividendDate), pay: mdy(d.dividendPaymentDate), annual: +(d.annualizedDividend || 0) || null, yieldPct: +(String(d.yield || '').replace('%', '')) || null, rows };
}
// Yahoo: dividend history for the last 2y (fallback + frequency)
async function yahooDividends(sym) {
  const j = await getJSON(YF + encodeURIComponent(sym) + '?range=2y&interval=1d&events=div');
  const r = j && j.chart && j.chart.result && j.chart.result[0];
  const ev = (r && r.events && r.events.dividends) || {};
  return Object.values(ev).map((x) => ({ ex: x.date * 1000, amount: +x.amount })).sort((a, b) => a.ex - b.ex);
}
module.exports.nasdaqDividends = nasdaqDividends;
module.exports.yahooDividends = yahooDividends;
